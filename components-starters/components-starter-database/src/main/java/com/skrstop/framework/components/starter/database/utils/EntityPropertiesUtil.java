...
public class EntityPropertiesUtil {
    ...
    public static Set<String> getColumnPropertyNames(Map<Class<?>, Map<String, Pair<String, Class<?>>>> propertyFieldCache, Class<?> propertyClass) {
        ...
        for (Field field : allFields) {
            ...
            if (ObjectUtil.isNotNull(propertyCreateBy)) {
                propertyFieldCache.computeIfAbsent(PropertyCreateBy.class, k -> new HashMap<>())
                        .put(field.getName(), Pair.of(targetFieldName, field.getType()));
                continue;
            }
            // 创建时间

package com.jphoebe.framework.components.starter.redis.redisson;
...
@Configuration
@ConditionalOnProperty(value = GlobalConfigConst.REDISSON_PREFIX + ".enabled", havingValue = "true", matchIfMissing = true)
public class RedissonAutoConfiguration {

    @Bean(destroyMethod = "shutdown")
    @ConditionalOnMissingBean(RedissonClient.class)
    public RedissonClient redissonClient(RedissonProperties redissonProperties, ApplicationContext ctx) throws AuntecException, IOException {
        Config config = null;
        ...
        if (ObjectUtil.isNull(config)) {
            throw new AuntecException("redisson未配置相关参数");
        }

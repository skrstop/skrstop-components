...
public abstract class DsSelector {
     ...
     * @return 数据源名称
     */
    public String determineConnectionFactory(MethodInvocation invocation, String key) {
        if (matches(key)) {
            String datasource = doDetermineConnectionFactory(invocation, key);
            if (datasource == null && nextDsSelector != null) {
                return nextDsSelector.determineConnectionFactory(invocation, key);
            }
            return datasource;
        }
     ...
     * @return 数据源名称
     */
    public abstract String doDetermineConnectionFactory(MethodInvocation invocation, String key);
